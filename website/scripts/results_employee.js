...
pairs.forEach(function (pair) {
  ...
  } else if (keyValue[0] === "userNas") {
    userNas = decodeURIComponent(keyValue[1]);
  }
});
...
var checkButtonInterval = setInterval(function () {
  ...
  if (bookButton) {
    bookButton.addEventListener("click", function () {
      ...
      var userNas = params.get("userNas");

      var xhr = new XMLHttpRequest();
      xhr.open("POST", "http://127.0.0.1:3000/reserve_employee", true);
      ...
      xhr.send(
        `chambre_id=${chambre_id}&date_start=${date_start}&date_end=${date_end}&userNas=${userNas}`
      );
